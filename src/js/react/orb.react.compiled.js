...
var pivotUtil = require( '../../../../pivotTable/pivotUtil')
...
module.exports.PivotTable = react.createClass({
	...
	moveAfield:(fieldname,oldAxeType, newAxeType, position) => {
		if (this.pgridwidget.moveField(fieldname, oldAxeType, newAxeType, position)) {
            this.setProps({});
        }
	},
    moveButton: function(button, newAxeType, position) {
        if(
		(button.props.axetype==axe.Type.DATA)
		||(button.props.axetype!=axe.Type.DATA && newAxeType==axe.Type.DATA)
		...
		//
		let tab=this.pgridwidget.originConfig.getCurrentTab();
		if("charts"==tab && this.pgridwidget.pgrid.config.rowFields.length>0){
			if (this.pgridwidget.replaceField(button.props.field.name, button.props.axetype, newAxeType, position)) {
				this.setProps({});
			}
		}else{
    ...
    synchronizeCompsWidths: function() {
		...
		//scroll zzj 20+
        var rowHeadersTableWidth = 20+Math.max(nodes.rowHeadersTable.size.width, rowButtonsContainerWidth, 67);
    ...
    render: function() {
		...
		let analyzePaneOpened=this.pgridwidget.originConfig.analyzePaneOpened;
		const collapseMap={
			true:{display:"",className:"upDownIcon downIcon",img:imgDirPrefix+"images/less.svg"},
			false:{display:"none",className:"upIcon upDownIcon",img:imgDirPrefix+"images/more_unfold.svg"}
		};
		let collapseFunc= 
			(e)=>{
						...
						this.pgridwidget.originConfig.analyzePaneOpened=!this.pgridwidget.originConfig.analyzePaneOpened;
						let cobj=collapseMap[this.pgridwidget.originConfig.analyzePaneOpened];
						document.getElementById("tbl-99").style.display= cobj.display;
						let upDownButton=document.getElementById("upDownButton");
						//默认是展开状态
							upDownButton.src=cobj.img;
							upDownButton.className=cobj.className;
							//document.getElementById("toggleDiv12").style.display="";
						
					}
		
		;
		let cobj=collapseMap[analyzePaneOpened];
        let panewidth=BUTTON_IN_ROW*getMaxDimensionBtnLength(config.rowFields,config.dataFields)+50;
        
        panewidth=panewidth >300 ? panewidth : 300;
        ...
        return (
            React.createElement("div", {
				...
				,React.createElement("tr", {
                    className: "toggleDiv1",
					id:"toggleDiv1",
					//style:{width:"100px"}
                    //onDoubleClick :collapseFunc
				} 
				,React.createElement("td", {
                    className: "toggleDiv12",
					id:"toggleDiv12",
					style:{display1: "inline-block",width:"80%" },
				//	float:"right",
                    onDoubleClick :collapseFunc
				} ,"分析面板")
				
				,React.createElement("td", {
                    className: "toggleDiv13",
					id:"toggleDiv13",
					style:{display1: "inline-block",width:"80%",align:"right",verticalAlign:"center" },
				//	float:"right",
                    //onClick :collapseFunc
				} 
				,React.createElement("img", {
					//src:imgDirPrefix+"images/more_unfold.svg",
					className: "collpaseIcon1"
				...
				,React.createElement("table", {
                        id: 'tbl-99',
                        ref: "pivotWrapperTable1",
                        className: classes.table+ " tbl99",
                        style: {
                            //tableLayout: 'fixed'
							display: cobj.display
                        }
                    }
					
					, React.createElement("tr", null,//row
                            React.createElement("td", {
									className:"rowbuttontd1",
					...
					,React.createElement("tr", {//x
                            ...
                            React.createElement("td", {
								...
								,React.createElement("img",
								{
									src:imgDirPrefix+"images/delete.svg"
									,title:"删除"
									,className:"toolIcon"
									,onClick:this.pgridwidget.originConfig.delSelectTable
									,style:{cursor:"pointer","verticalAlign":"bottom"
                                    ,marginLeft:"20px",height:"16px"}
								})
						...
						,React.createElement("tr", {//x
                            ...
                            React.createElement("td", {
									className:"mySavedTable1"
									,style:{textAlign:"center"}
                                },React.createElement("select", {
									id:"mySavedTableSelect"
									,onChange:this.pgridwidget.originConfig.onSelectTable
									,className:"mySavedTableSelect"
                                })
...
module.exports.DropTarget = react.createClass({
    ...
    render: function() {
		...
		//zzj
		var maxtd=BUTTON_IN_ROW;
		var trs=[];
		if(buttons.length>maxtd && !(self.props.nowrap)){
			var tmp=[];
			var count=0;
...
module.exports.PivotButton = react.createClass({
    ...
    onFilterMouseDown: function(e) {
        ...
        let oconfig=this.props.pivotTableComp.pgridwidget.originConfig;
        if(oconfig.onFilter && oconfig.onFilter(this.props.field,e,oconfig.fields)){
            //弹出自定义过滤组件
        }else{
        ...
        let csswidth=301;//orb.css fltr-scntnr
        ...
        filterContainer.style.left =pivotUtil.calcLeft(csswidth,filterButtonPos.x) + 'px';
    ...
    componentDidUpdate: function() {
        let movable=this.props.pivotTableComp.pgrid.config.canMoveFields && (this.props.movable==null ||this.props.movable==true);
        if (movable) {
 ...
 console.log("onMouseDown");
    ...
    onMouseUp: function(e) {
        ...
        if (!e.ctrlKey && !isdragged && "dimesionCheckbox"!=e.target.name
			&& "measureFuncBtn"!=e.target.name) {
                console.log("sort");
 ...
 console.log("onMouseMove");
    ...
    render: function() {
        ...
        if(self.props.btnSize){
            divstyle.width= self.state.size==null ? 
                self.props.btnSize : 
                (self.state.size.width<self.props.btnSize ? self.props.btnSize : self.state.size.width);
        }
		//divstyle.width="50px";
		//console.log("pivotButton",this.props);
        ...
        var sortDirectionClass = self.props.field.sort.order === 'asc' ?
            ...
            (self.props.field.sort.order === 'desc' ?
                ...
                //' \u2193' :
                '');
        let emptySortClass= (sortDirectionClass==null || sortDirectionClass.length==0) ? "emptySortClass" :"";
        ...
        if (self.props.axetype === axe.Type.DATA) {
			...
			sorttd=React.createElement("td", null, React.createElement("div", {
                            className: 'sort-indicator '+emptySortClass+" " + sortDirectionClass
                        }))
		}
		let checkboxs=null;
		// if (self.props.axetype != axe.Type.DATA) {
			 checkboxs=React.createElement("td",{}
						,React.createElement("input",{
							type:"checkbox"
							,value:self.props.field.name
							,checked:this.state.checked
							,onChange:(e)=>{
								////console.log("onchange ",e);
								this.setState({...this.state,checked:e.target.checked});
							}
							,name:"dimesionCheckbox"
							, className: "fieldCheckbox"
						})
						)
		// }
		 let methodFilterBtn=null;
		 if (self.props.axetype === axe.Type.DATA) {
			 let obj=pivotUtil.findObject(self.props.pivotTableComp.pgridwidget.originConfig.fields,self.props.field.name);
			 if(false || obj!=null && !pivotUtil.isEmpty(obj.CALTYPES) && obj.CALTYPES.length>1){
				methodFilterBtn= React.createElement("td", {
                                
                            },
                            React.createElement("div", {
                                //ref: "filterButton",
                                className: "measureFuncBtn",
								//src:"images/func.svg",
								name:"measureFuncBtn",
                                onMouseDown: self.state.dragging ? null : (e)=>{
									//if(self.props.pivotTableComp.pgridwidget.originConfig.filterFuncs)
										self.props.pivotTableComp.pgridwidget.originConfig
										.filterFuncs(e,{...self.props.field}
										,self.props.pivotTableComp.pgridwidget.originConfig);
                ...
                React.createElement("tbody", null,
                    React.createElement("tr", null,
                        ...
                        React.createElement("td", {
							...
							,
                            React.createElement("img", {
                                src:imgDirPrefix+"images/whiteClose.svg"
                                ,className: "closeButton",
                                width:"15px"
								,onClick:(e)=>{
									console.log("delet clicke");
									//this.props.pivotTableComp.moveAfield('department',1);
									if (this.props.pivotTableComp.pgridwidget.moveField(this.props.field.name, this.props.axetype)) {
										this.props.pivotTableComp.setProps({});
									}
								}
                            })
...
module.exports.PivotTableUpperButtons = react.createClass({
    render: function() {
        ...
        if (config.canMoveFields) {
            var btnSize=getMaxDimensionBtnLength(config.rowFields,config.availablefields());
            ...
            fieldsDropTarget = React.createElement("tr", 
			{
				id:"leftFieldTr1"
				,className:"leftFieldTrs"
				,style:{display:""}
			}
			,
        ...
        var btnSize=getMaxMeasureBtnLength(config.dataFields); 
        ...
        var dataDropTarget = React.createElement("tr", 
			{className:"measuresTr"},
			/*
			React.createElement("td", {
...
module.exports.PivotTableColumnButtons = react.createClass({
    render: function() {
        ...
        var config = this.props.pivotTableComp.pgridwidget.pgrid.config;
        var btnSize=getMaxDimensionBtnLength(config.rowFields,config.columnFields);
...
var BUTTON_IN_ROW=2;
//获取维度按钮的长度
function getMaxDimensionBtnLength(rowFields,columnFields){
    let textLength=0;
    if(rowFields!=null)
    rowFields.forEach((o)=>{
        if(o.caption && o.caption.length>textLength)textLength=o.caption.length;
        // console.log("btn caption",o.caption,o.caption.length);
    });
     if(columnFields!=null)
    columnFields.forEach((o)=>{
        if(o.caption && o.caption.length>textLength)textLength=o.caption.length;
    });
    let r= getWidthByTextLength(textLength)+65;
   // console.log("btn width",r);
    return r>0 ? r :110;
}
function getWidthByTextLength(textLength){
    return textLength*15;
}

//获取维度按钮的长度
function getMaxMeasureBtnLength(dataFields){
    let textLength=0;
    if(dataFields!=null)
    dataFields.forEach((o)=>{
        if(o.caption && o.caption.length>textLength)textLength=o.caption.length;
    });
...
module.exports.PivotTableColumnHeaders = react.createClass({
    render: function() {
        ...
        return React.createElement("div", {
            ...
            React.createElement("table", {
                    className: "inner-table topCaptionTable"
                },
                React.createElement("colgroup", null),
                React.createElement("tbody", null,
                    columnHeaders
...
module.exports.PivotTableRowHeaders = react.createClass({
    ...
    render: function() {
        ...
        return React.createElement("div", {
            ...
            React.createElement("table", {
                    className: "inner-table leftCaptionTable"
                },
...
module.exports.PivotTableDataCells = react.createClass({
    render: function() {
        ...
        return React.createElement("table", {
                className: "inner-table"+" pivotDataTable"
				//,style:"tr:nth-child(odd){background-color: rgba(0,0,0,.05)!important;  border-collapse:collapse!important;}"
            },
            React.createElement("colgroup", null),
            React.createElement("tbody", null,
                dataCells
...
module.exports.FilterPanel = react.createClass({
    ...
    render: function() {
        ...
        addCheckboxRow(filtering.ALL, '(所有)');
